...
public class CrowdTest implements Application{
	...
	private String crowdRecord(int crowdId){
		//获取群聊的当前信息
		CrowdRecordViewDAO viewDAO = 
				(CrowdRecordViewDAO)context.getBean("CrowdRecordViewDAO");
		List list = viewDAO.findByCrowdId(crowdId);
		log.debug("list.size:" + list.size() + "===================================");
		for(Iterator iterator = list.iterator();iterator.hasNext();){
			CrowdRecordView recordView = (CrowdRecordView)iterator.next();
			log.debug(recordView.getNickname() + ":" + recordView.getSendContent());
		}
		return null;
	}
	/**
	...
	public String saveCrowdRecord(){
		ChatRecordDAO chatRecordDAO = (ChatRecordDAO)context.getBean("ChatRecordDAOImpl");
	...
	public void getCrowdMenberInfo(){
		log.info("===========begin===============");
		MySessionDAO mySession = 
				(MySessionDAO)context.getBean("MySessionDAO");
		List list=
			mySession.getQuery("crowdMenberInfo")
			.setString(0, "10001")
			.setInteger(1, 10001).list();
		log.info("list size:" + list.size());
		for(Iterator iterator=list.iterator();iterator.hasNext();){
			//CrowdMenberInfo crowdMenberInfo = (CrowdMenberInfo)iterator.next();
	...
	public void getCrowdInfo(){
		...
		CrowdInfo crowdInfo = crowdInfos.get(0);
		log.info("crowdId" + crowdInfo.getCrowdId());
		log.info("crowdName:" + crowdInfo.getCrowdName());
		log.info("crowdNum:" + crowdInfo.getCrowdNum());*/
	}
	
	public void getHeadImage(){
		try{
			byte[] bytes = FileUtil.makeFileToByte("F:/image/well.jpg");
			new com.atm.chat.nio.server.handler.ImageHandler("jpg", bytes, 1, "10001").start();
		}catch(Exception e){
			log.info("error");
		}

...
<html>
  <head>
    <base href="<%=basePath%>">
    <jsp:useBean id="recuitBean" class="com.atm.model.define.RecuitView" scope="request"></jsp:useBean>
    <title><%= recuitBean.getReTypeName()+"-"+recuitBean.getWoTypeName() %></title>  
    ...
    <tr><td valign="top" width="60px">
     <!--头像-->
   	<div style="width:50px; height:50px; border-radius:10%; overflow:hidden; margin-left:0.3em; margin-top:0.3em">
        <a href="javascript:void(0)" onclick="toPerson('<%= recuitBean.getPublisherId() %>')"><img src="<%= recuitBean.getHeadImagePath() %>" alt="用户头像" 
...
name="user_head" width="100%" height="100%" id="user_head" style="background-color:#CF0;display:block;" /></a>
    ...
    <!--昵称及日期-->
  	<div>
            ...
            <a href="javascript:void(0)" onclick="toPerson('<%= recuitBean.getPublisherId() %>')">
				<font color="#000000"><%= recuitBean.getNickname() %></font>
                </a>
            </td></tr>
           <tr><td  style="font-size:0.7em;color:#A5A5A5; overflow:hidden;text-overflow:ellipsis; display:block; "> <%= recuitBean.getPublishTime() %>
		   			 <%-- <% if(me.equals(recuitBean.getPublisherId())) %> (应有删除按钮)  --%>
                </td>
                </tr>
           </table>
    ...
    <td style="min-width:6em"><!--最小宽度为下面的块保留空间，避免重叠 -->
    		<div  style="font-size:0.7em; position:absolute; right:0.1em; top:0.2em; text-align:center">
    			...
    			<p style="margin-top:-0.5em; color:#666">阅读(<%= recuitBean.getClickNum() %>)</p>
   			</div>
    </td>
    <!--阅读量结束-->
    </tr>
     ...
     <span class="recuitType"><%= recuitBean.getWoTypeName() %></span>
  ...
  <div class="content">
    ...
    <table>
        <tr>
        <td>薪资：</td><td colspan="1"><span class="salary"><%= recuitBean.getSalary() %></span></td>
        </tr>
        <tr>
        <td>招聘类型：</td><td><%= recuitBean.getReTypeName() %></td>
        </tr>
         <tr>
        <td>工作类型：</td><td><%= recuitBean.getWoTypeName() %></td>
        </tr>
         <tr>
        <td> 工作地点：</td><td><%= recuitBean.getWorkAddress() %></td>
        </tr>
        <tr>
        <td> 联系方式：</td><td><%= recuitBean.getTelephone() %></td>
        </tr>

...
<script type="text/javascript">
	...
	/* 	 $(window).scroll(function() {
         	...
         	//触屏事件
         var obj = document.getElementById("replyList");
			...
			obj.addEventListener("touchend", function(event) {
				$("#jump").delay(5000).hide(1000);//跳转块隐藏
			}, false);
	...
	function showJumpDiv(){
		...
		$("#jumpDiv")
		.css({"position":"fixed","left":(screenWidth-$("#jumpDiv").width())/2+"px","top":screenHeight/4}).show();
	}
	...
	function jump(){
		//点击确认按钮的方法
   		try{ 			
   			...
   			//如果是跳转到已经加载的页面
 			var dom = document.getElementById("reply_"+index);
 			if(dom!=null){ //若该元素为空说明未被加载过，需加载
   				dom.scrollIntoView();//定位到元素
   			...
   			beforeJumpIndex = replyIndex;//记录跳页前的索引，用来索引复原（跳的页数不存在时）
   			replyIndex = index;
   			getReply();
   		}catch(e){
   			showMess("查询错误"+e);
	...
	function addAReply(repliedUserId,floorId,repContent){
			if(repContent==null||repContent.replace(/ /g,"").length==0){
				showMess("请发送有内容的评论");
				return;
			}
			$.ajax({
	...
	function personPage(u){//跳转用户个人界面
		if(checkLogin()==false){
		 ...
		 window.commentView.toPersonalMsg(u);
	}
	//点赞方法
			...
			function succFunction(json){
				...
				if(json[0].num!=null){//没有更多评论
					if(json[0].num==0){//replyIndex==-1
						...
						/* $("#moreReply").hide(); */
						if(inJump==true){
							replyIndex = beforeJumpIndex; //无跳转，索引复原
							inJump = false;
					...
					if(json[0].tip!=null){
						...
						if(replyIndex==-1){
							...
							$("#replyList").append(aDiv);//将此评论显示
							//跳转到加载出来的评论
							if(replyIndex!=1&&index==0){
								document.getElementById("floor_"+floorId).scrollIntoView();
							}
							getInnerDiv(floorId);//获取楼中楼回复
						}
						replyIndex++; //索引+1
...
<div>
	...
	<div class="container" id="container">
		...
		<!-- <div class="refreshButton" id="refresh"></div> -->
		...
		<!-- <div class="refreshButton" id="moreReply"></div> -->
	</div>
	<div id="bottom"></div>
	<div id="jumpDiv">第<input type="text" id="page" size="11"/>页<input type="button" value="跳转" onclick="jump()" style="width:5em;height:2 em;"/></div>
	<div id="message"></div>
</div>

...
public class MessageService implements Application {
	 ...
	 * @return PrivateChat
	 */
	public static List<PrivateChat> getOffLineMessage(String userId) {
		PrivateChatDAO chatDAO = (PrivateChatDAO) context
				.getBean("PrivateChatDAOImpl");
		List<PrivateChat> chatList = chatDAO.findByUserReceiveId(userId);
		return chatList;
	}
	 ...
	 * @return ChatRecord
	 */
	public static List<ChatRecord> getOffLineCrowdMessage(String userId) {
		MySessionDAO mySessionDAO = (MySessionDAO) context
				.getBean("MySessionDAO");
		List<ChatRecord> list = mySessionDAO.getCrowdOffRecord(userId);
		for (Iterator<ChatRecord> iterator = list.iterator(); iterator.hasNext();) {
			ChatRecord chat = iterator.next();

 ...
 * 
 * @see com.atm.model.define.Login
...
public class LoginDAO {
	...
	public Login findById(java.lang.String id) {
		...
		try {
			Login instance = (Login) getCurrentSession().get(
					"com.atm.model.define.Login", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Login instance) {
		...
		try {
			List results = getCurrentSession()
					.createCriteria("com.atm.model.define.Login")
	...
	public List<Login> findLoginList(int first,int max){
		log.info("find userList with limit");
		try{
			List<Login> list = getCurrentSession().createCriteria(Login.class)
